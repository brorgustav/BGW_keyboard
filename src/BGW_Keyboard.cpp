...
|| | Keyboards. It supports multiple keypresses while maintaining
...
// <<constructor>> Allows custom keymap, pin configuration, and Keyboard sizes.
Keyboard::Keyboard(char *userKeymap, byte *row, byte *col, byte numRows, byte numCols) {
	rowPins = row;
	columnPins = col;
...
void Keyboard::begin(char *userKeymap) {
    keymap = userKeymap;
}
...
// New in 2.1
bool Keyboard::isPressed(char keyChar) {
	for (byte i=0; i<LIST_MAX; i++) {
...
int Keyboard::findInList (char keyChar) {
	for (byte i=0; i<LIST_MAX; i++) {
		if (key[i].kchar == keyChar) {
			return i;
		}
...
|| | 3.0 2012-07-12 - Mark Stanley     : Modified pin functions to support Keyboard_I2C
|| | 3.0 2012-07-12 - Stanley & Young  : Removed static variables. Fix for multiple Keyboard objects.
...
|| | 1.7 2009-06-18 - Alexander Brevig : Every time a state changes the KeyboardEventListener will trigger, if set.
